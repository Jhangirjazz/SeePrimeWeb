...
<div class="scrollable-hero-banner">
    <div class="hero-banner-scroll-inner">
        @foreach (collect($grouped->flatten(1))->take(4) as $item)
            @php
                ...
                $source = strtolower($item['SOURCE'] ?? $item['SOURCE_PATH'] ?? '');
                    ...
                    // ✅ Fix: Extract YouTube video ID if full URL is stored
                    ...
                    if ($thumb && (str_ends_with($thumb, '.jpg') || str_ends_with($thumb, '.png'))) {
                    ...
                    // rest of your code...
                    $detailApi = "http://15.184.102.5:8443/SeePrime/APIS/SELECT.php?select_id=content_detail&content_id={$contentId}&admin_portal=Y";
                    ...
                    $trailerUrl = $videoPath ? "http://15.184.102.5:8443/SeePrime/Content/Videos/{$videoPath}" : '#';
        @endphp
...
<section class="mb-4 px-3">
    ...
    @foreach ($continue as $c)
        <div class="me-3 continue-card">
            <a href="{{ route('play.video', ['id' => $c->video_id]) }}">
                <div class="thumbnail-wrapper position-relative rounded shadow-sm border border-danger" style="overflow:hidden;">
                    <img src="{{ $c->thumb ?: asset('images/default.jpg') }}"
                         class="rounded w-100"
                         alt="{{ $c->title }}"
                         style="height: 150px; object-fit: cover;">

                    <div class="resume-badge position-absolute top-0 end-0 px-2 py-1 bg-danger text-white small fw-bold">
                        ⏯ Continue
                    </div>
            ...
            <br>
            <span class="d-inline-block text-light mb-1" style="font-size: 13px;">From</span><br>
    ...
    <div class="scroll-container">
        @foreach ($latest as $item)
            @php
                ...
                if ($thumb && str_ends_with($thumb, '.jpg') || str_ends_with($thumb, '.png')) {
                    $thumbUrl = "http://15.184.102.5:8443/SeePrime/Content/Images/{$thumb}";
                } elseif ($source === 'youtube' && $thumb && str_contains($thumb, 'youtube.com')) {
                    parse_str(parse_url($thumb, PHP_URL_QUERY), $ytParams);
                    $thumb = $ytParams['v'] ?? $thumb;
                    $thumbUrl = "https://img.youtube.com/vi/{$thumb}/maxresdefault.jpg";
                } elseif ($source === 'youtube' && $thumb) {
            ...
            <div class="content-card">
                <a href="{{ route('play.video', ['id' => $item['CONTENT_ID']]) }}">
                    <div class="thumbnail-wrapper">
                        <img src="{{ $thumbUrl }}" alt="{{ $item['TITLE'] }}" class="rounded w-100">
                    </div>
...
<div class="trending-banner position-relative text-white py-5"
     style="background: linear-gradient(to right, rgba(0,0,0,0.85), rgba(0,0,0,0.4)),
            url('http://15.184.102.5:8443/SeePrime/Content/Images/password.jpg') center center / cover no-repeat;">
...
let allSelected = false; // 🔁 State to toggle
...
document.getElementById('selectAllFilters')?.addEventListener('click', () => {
    ...
    // ✅ Instead of relying on arrays, get .active buttons live
    ...
    // Optional: update arrays (if used elsewhere)
...
window.alert = function(message) {
    ...
    const toastBody = document.getElementById('alertToastMessage');
    ...
    const scrollContainer = document.querySelector('.scrollable-hero-banner');
    ...
    if (scrollContainer && slides.length > 0) {
        ...
        setInterval(() => {
            ...
            scrollContainer.scrollTo({ left: index * slideWidth, behavior: 'smooth' });
        }, 5000);
    }
